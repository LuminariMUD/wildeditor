...
Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates?x=200&y=300" -Headers @{"X-API-Key" = $API_KEY}

# Test 4: Batch terrain data (small area)
Invoke-RestMethod -Uri "$BASE_URL/api/terrain/batch?x_min=0&y_min=0&x_max=5&y_max=5" -Headers @{"X-API-Key" = $API_KEY}

# Test 5: Batch terrain data (larger area for mapping)
Invoke-RestMethod -Uri "$BASE_URL/api/terrain/batch?x_min=-10&y_min=-10&x_max=10&y_max=10" -Headers @{"X-API-Key" = $API_KEY}

# Test 6: Test coordinate bounds (should work)
Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates?x=1024&y=-1024" -Headers @{"X-API-Key" = $API_KEY}

# Test 7: Test coordinate bounds (should fail gracefully)
try {
    Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates?x=1025&y=0" -Headers @{"X-API-Key" = $API_KEY}
} catch {
...
Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms?limit=10" -Headers @{"X-API-Key" = $API_KEY}

# Test 9: List more wilderness rooms
Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms?limit=50" -Headers @{"X-API-Key" = $API_KEY}
...
Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms/1000123" -Headers @{"X-API-Key" = $API_KEY}

# Test 12: Test room that might not exist
try {
    Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms/9999999" -Headers @{"X-API-Key" = $API_KEY}
} catch {
...
try {
    Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates" -Headers @{"X-API-Key" = $API_KEY}
} catch {
...
try {
    Invoke-RestMethod -Uri "$BASE_URL/api/terrain/batch?x_min=-100&y_min=-100&x_max=100&y_max=100" -Headers @{"X-API-Key" = $API_KEY}
} catch {
...
$largeBatch = Invoke-RestMethod -Uri "$BASE_URL/api/terrain/batch?x_min=0&y_min=0&x_max=31&y_max=31" -Headers @{"X-API-Key" = $API_KEY}
...
function Get-TerrainSummary {
    ...
    $terrain = Invoke-RestMethod -Uri "$BASE_URL/api/terrain/batch?x_min=$MinX&y_min=$MinY&x_max=$MaxX&y_max=$MaxY" -Headers @{"X-API-Key" = $API_KEY}
...
function Get-ElevationProfile {
    ...
    for ($x = $StartX; $x -le $EndX; $x++) {
        $terrain = Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates?x=$x&y=$StartY" -Headers @{"X-API-Key" = $API_KEY}
...
function Get-ZoneEntrances {
    $rooms = Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms?limit=100" -Headers @{"X-API-Key" = $API_KEY}
...
function Run-QuickTests {
    ...
    try {
        $terrain = Invoke-RestMethod -Uri "$BASE_URL/api/terrain/at-coordinates?x=0&y=0" -Headers @{"X-API-Key" = $API_KEY}
    ...
    try {
        $rooms = Invoke-RestMethod -Uri "$BASE_URL/api/wilderness/rooms?limit=5" -Headers @{"X-API-Key" = $API_KEY}
