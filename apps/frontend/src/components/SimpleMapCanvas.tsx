...
export const SimpleMapCanvas: FC<SimpleMapCanvasProps> = ({
  ...
  const render = useCallback(() => {
    ...
    const displaySize = CANVAS_SIZE * (state.zoom / 100);
  ...
  // Mouse handlers
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    const gameCoord = mouseToGame(e.clientX, e.clientY);
    onMouseMove(gameCoord);
  }, [mouseToGame, onMouseMove]);
    ...
    if (state.tool === 'select') {
      ...
      for (const path of paths) {
        if (path.coordinates.length >= 2) {
          ...
          if (minDistance <= 15) { // 15 game unit tolerance
            onSelectItem(path);
            return;
          }
        }
