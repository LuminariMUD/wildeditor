...
export WILDEDITOR_MCP_KEY=<redacted>
export WILDEDITOR_MCP_BACKEND_KEY=<redacted>
...
curl -H "X-API-Key: <redacted>" http://localhost:8001/health/detailed
```

### **Capability Testing**
```bash
# Test tool availability
curl -H "X-API-Key: <redacted>" http://localhost:8001/mcp/tools

# Test resource access
curl -H "X-API-Key: <redacted>" http://localhost:8001/mcp/resources

# Test backend connectivity (will show errors if backend not available)
curl -H "X-API-Key: <redacted>" -H "Content-Type: application/json" \
