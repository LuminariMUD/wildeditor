...
MYSQL_DATABASE_URL=mysql+pymysql://user:<redacted>@host/database
...
MYSQL_DATABASE_URL=mysql+pymysql://user:<redacted>@luminari-mysql-host:port/database
...
python -c "
from sqlalchemy import create_engine
engine = create_engine('mysql+pymysql://user:<redacted>@host/db')
engine.connect()
