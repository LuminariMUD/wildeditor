...
- **Temporary (per shell):**
  ```sh
  export MYSQL_DATABASE_URL='mysql+pymysql://user:<redacted>@host:port/dbname'
  ```
- **Persistent (per user):**
...
- **systemd service:**
  ...
  ```ini
  [Service]
  Environment="MYSQL_DATABASE_URL=mysql+pymysql://user:<redacted>@host:port/dbname"
  ```

### Windows (PowerShell or System Environment)
- **Current user:**
  ```powershell
  [System.Environment]::SetEnvironmentVariable("MYSQL_DATABASE_URL", "mysql+pymysql://user:<redacted>@host:port/dbname", "User")
  ```
- **System-wide:**
  ```powershell
  [System.Environment]::SetEnvironmentVariable("MYSQL_DATABASE_URL", "mysql+pymysql://user:<redacted>@host:port/dbname", "Machine")
  ```
- **GUI:**
